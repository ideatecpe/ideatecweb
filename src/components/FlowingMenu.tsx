...
interface MenuItemComponentProps extends MenuItemProp {
  speed: number;
  textColor: string;
  marqueeBgColor: string;
  marqueeTextColor: string;
...
function MenuItem({
  ...
  const SelectedIcon = icon ? IconMap[icon] : Hash;
    ...
    <div className="menu__item" ref={itemRef} style={{ borderColor }}>
      ...
      >
        <div className="flex items-center w-full px-5 py-2">
          <div className="w-8 h-8 rounded-lg bg-white/10 border border-white/20 flex items-center justify-center text-white mr-4 shrink-0 shadow-sm">
            <SelectedIcon className="w-4 h-4" />
          </div>
          <div className="flex flex-col items-start text-left">
            {label && (
              <span className="text-[9px] uppercase tracking-wider text-gray-200 font-bold leading-tight">
                {label}
              </span>
            )}
          ...
          <div className="marquee__inner" ref={marqueeInnerRef} aria-hidden="true">
            {[...Array(repetitions)].map((_, idx) => (
              <div className="marquee__part " key={idx} style={{ color: marqueeTextColor }}>
                <span className="!text-[12px]">{label ? `${label}: ${text}` : text}</span>
